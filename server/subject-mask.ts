...
/**
 * Người đứng LỆCH bao nhiêu so với tâm khung, theo tỉ lệ bề rộng.
 *
 * `0` là đứng giữa, `-0,2` là lệch trái một phần năm khung.
 *
 * Cần vì phép thu video vào một Ô luôn cắt GIỮA: nguồn 9:16 nhét vào một ô gần
 * vuông thì mất hai bên, và nếu người đứng lệch thì mặt bị xén. Đo được trên
 * một bản: người lệch trái 0,08 khung, ô vuông cắt mất nửa má phải.
 *
 * Đọc từ mặt nạ nên nó biết NGƯỜI ở đâu, không đoán theo tâm hình học.
 */
export async function subjectOffset(
  projectId: string,
  atSecond: number,
): Promise<number> {
  if (!hasSubject(projectId)) return 0;
  try {
    /*
     * Chia khung làm năm cột, hỏi cột nào đục nhất rồi lấy trọng tâm có trọng số.
     *
     * Năm cột là đủ: ta cần biết "lệch trái hay phải bao nhiêu", không cần biết
     * mép người ở điểm ảnh nào. Và năm lượt ffmpeg đọc một khung thì rẻ.
     */
    const cols = 5;
    ...
    for (let index = 0; index < cols; index += 1) {
      const { stderr } = await run(
        ...
        ["-hide_banner", "-nostats", "-ss", String(atSecond), "-i", subjectPath(projectId),
         "-frames:v", "1",
         "-vf", `crop=iw/${cols}:ih:iw*${index}/${cols}:0,signalstats,` +
                `metadata=print:key=lavfi.signalstats.YAVG`,
         "-f", "null", "-"],
        { timeout: 30_000 },
      ).catch((e: { stderr?: string }) => ({ stderr: e.stderr ?? "" }));
      const match = /YAVG=([\d.]+)/.exec(String(stderr));
      weights.push(match ? Number(match[1]) : 0);
    }
    const sum = weights.reduce((a, b) => a + b, 0);
    if (sum <= 0) return 0;
    // Trọng tâm theo cột, quy về tỉ lệ lệch so với giữa khung.
    const centre = weights.reduce((acc, w, i) => acc + w * ((i + 0.5) / cols), 0) / sum;
    return Math.max(-0.35, Math.min(0.35, centre - 0.5));
  } catch {
    return 0;
  }
}
